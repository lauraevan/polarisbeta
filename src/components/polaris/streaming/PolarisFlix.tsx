...
function AiMatch({ onSelect }: { onSelect: (item: TmdbItem, kind: MediaKind) => void }) {
  ...
  async function suggest() {
    ...
    try {
      ...
      // Resolve each suggestion via TMDB search
      ...
      <div className="mx-auto max-w-4xl">
        <div className="mb-6 rounded-2xl border border-white/10 bg-black/40 p-5 backdrop-blur-md sm:p-7">
          <div className="mb-1 flex items-center gap-2 text-[10px] uppercase tracking-[0.3em] text-white/65">
            <Wand2 className="h-3.5 w-3.5" /> AI Match
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            {results.map((r) => {
              ...
              return (
                <button
                  key={key}
                  onClick={() => onSelect(r.item, r.kind)}
